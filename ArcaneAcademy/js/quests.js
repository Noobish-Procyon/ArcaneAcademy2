...
function completeQuest(questName) {
  ...
  save.quests[questName].completed = true;
  save.gold += quest.rewardGold;
  gainXp(quest.rewardXp);
  if (quest.rewardItem) save.inventory.push(quest.rewardItem);

  log(`Quest complete! +${quest.rewardGold} gold, +${quest.rewardXp} XP, Item: ${quest.rewardItem}`);
  updateUI();
  saveGame();
}
