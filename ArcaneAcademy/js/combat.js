...
function generateQuestion(subject) {
  if (subject === "math") {
    ...
    return { text: `What is ${a} + ${b}?`, answer: String(a + b) };
  }
  if (subject === "science") {
    return { text: "What gas do humans breathe in to survive?", answer: "oxygen" };
  }
  if (subject === "reading") {
    return { text: "Fill in the blank: The cat sat on the ____.", answer: "mat" };
  }
  if (subject === "coding") {
    return { text: "In JavaScript, what keyword declares a variable? (var/let/const)", answer: "let" };
  }
  return { text: "Answer 1 to cast your spell.", answer: "1" };
}
...
function winBattle() {
  ...
  gainXp(25);
  if (Math.random() < 0.4) addItem("Potion");
